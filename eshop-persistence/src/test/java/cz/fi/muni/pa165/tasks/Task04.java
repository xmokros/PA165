...
public class Task04 extends AbstractTestNGSpringContextTests {
	...
	public void findProductByName(){
		List<Product> found = em.createQuery("SELECT p FROM Product p WHERE p.name like :name",Product.class)
	 ...
	 * 
	 * This is very interesting task because it should help you understand how ON keyword differs from standard WHERE.
	...
	public void leftJoinCagesWithPets() {
		...
		List<ProductAndCategory> productAndCategory =
				em.createQuery("SELECT NEW cz.fi.muni.pa165.tasks.ProductAndCategory(p, c) FROM Product p " +
						"LEFT JOIN p.categories c ON c.name = 'Kitchen' ORDER BY p.name, c.name",ProductAndCategory.class).getResultList();
	...
	public void criteriaFindAll(){
		...
		Root<Product> roots = query.from(Product.class);
		query.select(roots);
	...
	public void criteriaFindProductsWithNonEmptyCategory() {
		...
		Root<Product> roots = query.from(Product.class);
		query.select(roots).where(cb.isNotEmpty(roots.<List<Category>>get("categories")));
		List<Product> found = em.createQuery(query).getResultList();
		Assert.assertEquals(found.size(), 3);
	}
}
